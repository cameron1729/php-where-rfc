...
// https://github.com/cameron1729/php-src/tree/where-syntax
...
$inrange = fn(stdClass $product): bool where ($price = $product->cost * 1.2) => $price <= 10 && $price > 5;
...
$sumpairs = fn(array $pair): int where([$a, $b] = $pair, $sum = $a + $b) => $sum;
...
$fullnameandslug = fn(array $user): array where ($fullname = $user['first'] . " " . $user['last']) => [
    $fullname,
